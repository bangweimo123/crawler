...
import cn.mbw.crawler.core.processor.plugins.downloader.ToutiaoDownloader;
import cn.mbw.crawler.core.processor.plugins.entity.CrawlerConfig;
import cn.mbw.crawler.core.processor.plugins.filters.BaseImgCrawlerFilter;
...
import cn.mbw.crawler.core.processor.TestPageProcessor;
import cn.mbw.crawler.core.processor.converter.HtmlTagFilterConverter;
import cn.mbw.crawler.core.processor.converter.StringTrimCrawlerConverter;
import cn.mbw.crawler.core.processor.iface.ICrawlerFilter;
import cn.mbw.crawler.core.processor.plugins.proxy.DefaultSite;
import cn.mbw.crawler.core.processor.plugins.filters.CrawlerFilterFactory;
import cn.mbw.crawler.core.processor.spider.SpiderFactory;
import cn.mbw.crawler.core.processor.utils.DomainTagUtils;
...
public class KuafuCrawlerTest extends BaseTest {
    ...
    @Test
    public void testboohee2() throws InterruptedException, InvocationTargetException, IllegalAccessException {
        InstanceFactory.getInstance("kuafuCrawlerJedisProviderFactoryBean");
        ICrawlerConfigParser crawlerConfigParser = new JsonCrawlerConfigParser();
        CrawlerConfig config = crawlerConfigParser.parser("boohee");
        SpiderFactory.initSpider(config);
        SpiderFactory.startSpider(config.getDomainTag());
        while (true) {
            if (SpiderFactory.getSpider(config.getDomainTag()).spiderIsRun()) {
                System.out.println(123);
                Thread.sleep(1000l);
            }
        }
    ...
    @Test
    public void testboohee() throws InterruptedException, InvocationTargetException, IllegalAccessException {
        boolean batchFlag = true;
        InstanceFactory.getInstance("kuafuCrawlerJedisProviderFactoryBean");
        ICrawlerConfigParser crawlerConfigParser = new JsonCrawlerConfigParser();
        CrawlerConfig config = crawlerConfigParser.parser("boohee-unit");
        String baseDomainTag = config.getDomainTag();
        String baseUrl = config.getCrawlerBaseInfo().getBaseUrl();
        Integer size = (Integer) config.getCustomParams().get("size");
        Integer batch = 1;
        Integer simpleStart = 0;
        try {
            for (int i = 0; i < 50; i++) {
                Integer start = (i * size + (batch - 1) * 50 * size) + 1 + simpleStart;
                CrawlerConfig currentConfig = JSON.parseObject(JSON.toJSONString(config), CrawlerConfig.class);
                Map<String, Object> customParams = new HashMap<>();
                customParams.put("size", size);
                customParams.put("start", start);
                customParams.put("batch", batch);
                currentConfig.setCustomParams(customParams);
                currentConfig.setDomainTag(baseDomainTag + start);
                currentConfig.getCrawlerBaseInfo().setBaseUrl(baseUrl + start);
                SpiderFactory.initSpider(currentConfig);
                SpiderFactory.startSpider(currentConfig.getDomainTag());
            }
        } finally {
            while (batchFlag) {
                boolean isAllFinish = true;
                for (int j = 0; j < 50; j++) {
                    Integer start = (j * size + (batch - 1) * 50 * size) + 1 + simpleStart;
                    if (SpiderFactory.getSpider(baseDomainTag + start).spiderIsRun()) {
                        isAllFinish = false;
                        Thread.sleep(10000l);
                        break;
                    }
